...
public class Sprite {
	 ...
	 * @param a The animation to use for the sprite.
	 */
	public void setAnimation(Animation a, boolean b)
	{
		anim = a;
		change = b;
	}
	...
	public void draw(Graphics2D g)
	{
		...
		if (change == true)
		{
			g.drawImage(getImage(),(int)x+xoff-27,(int)y+yoff,null);
		}
		else 
		{
	 ...
	 */
	public void drawBoundingBox(Graphics2D g, int i, int j, int k, int l)
	{ 
		if (!render) return;
		//System.out.println("i: "+i+" j: "+j+" k: "+k+" l: "+l);
			Image img = getImage();
			g.drawRect((int)x+xoff+i,(int)y+yoff+j,img.getWidth(null)+k,img.getHeight(null)+l);		
	}
	...
	public void drawTransformed(Graphics2D g)
	{
		...
		transform.translate(Math.round(x)+shiftx+xoff-34,Math.round(y)+shifty+yoff);
